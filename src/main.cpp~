...
const double ode_eps_rel0 = 1E-7; // relative error tolerance for first round ode integrator (1E-7) used in mode 1
const double ode_eps_rel1 = 1E-10; // relative error tolerance for second round ode integrator (1E-10) used in mode 1
int fit_iter;
...
const double fit_eps_rel = 1E-4; // the relative error tolerance at the fitting point in mode 1 round 2 (1E-4). Should be four orders of magnitudes larger than ode_eps_rel1.
...
int main()
{
  ...
  // 3: modify a built-in EOS, 
  // 4: iterate over EOS modifications with two-layer sovler, 5: iterate over EOS with regular solver
  // 6: bulk input mode with regular solver
  // 7: composition finder, secant method to third layer to match a mass and radius measurement
  ...
  if (input_mode == 0)
  {
	...
	cout<<Mcomp[i]<<", ";
      ...
      planet->print("Structure.txt", true);
...
// generate planet model by given the core mass, mantle mass, and ice layer mass, gas mass, all in earth mass unit.
    if (planet)
      planet->print("Structure.txt");
  ...
  else if(input_mode == 4)
  {
    ...
    twolayer(index,fraction,P_surface,1,"PosteriorsPPv.txt", "PPvMCMC.txt");
  ...
  else if(input_mode == 5)
  {
    ...
    fullmodel(Comp,Mcomp,Tgap,ave_rho,P_surface,false,2,"PosteriorsVinet.txt", "IceMCMCnew.txt");
  }
